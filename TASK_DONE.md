...
### ✅ 2026-09-21 일반 사용자 행번 비표시

| ID | 상태 | 조치 | 근거 |
| --- | --- | --- | --- |
| FE-94 | ✅ Done | 일반 화면의 이름(행번)·행번 열·이름 누락 폴백 제거. 관리자 전용 화면 및 내부 식별자 유지. 일반 조회용 Excel/PDF 행번 제거. 전산업무비 업로드 양식의 담당자사번 유지(사용자 확정) | [설계](docs/design-docs/2026-09-21-employee-number-visibility.md) · [검증](docs/test-docs/2026-09-21-employee-number-visibility.md) |

- check·format 통과. 전체 6,113건 실행 후 이전 폴백 기대 3건 갱신, 최종 관련 237건·브라우저 2건 통과. 프론트엔드 main 2cdb86df push 완료. 내부망 반입 세트 생성, 운영 배포 미실시.

### ✅ 2026-09-21 첨부 파일명·저장 실패 처리

| ID | 상태 | 조치 | 근거 |
| --- | --- | --- | --- |
| FE-93 | ✅ Done | 공통 첨부 선택 및 사업 가이드·요구사항정의서 본문 파일/이미지의 확장자 포함 UTF-8 100바이트 사전 검사, 사업·비용 저장·첨부 삭제 전 검증. 사업 첨부 실패 시 완료·이동 차단, 본문 저장 안내와 실패 파일·관리번호 보존 | [설계](docs/superpowers/specs/done/2026-09-21-attachment-name-and-save-failure-design.md) · [분석설계서](docs/design-docs/2026-09-21-analysis-design-since-260921.md) · [결과서](docs/test-docs/2026-09-21-test-result-since-260921.md) |

- 검증: 에디터 확장 후 전체 Vitest 6,092건 통과. 기존 예산 E2E 7건 및 추가 에디터 E2E 1건 통과. check·format:check 통과. 실DB 시나리오 미실시.
- 프론트엔드 main 89d29f06 커밋·push 완료. 운영 서버 배포 미실시. 테스트 결과서·PDF·증적은 기존 로컬 산출물 정책 적용.
