...
| --- | :------: | ---- | ---- | --------- | --------- |
| COUNCIL-014 | 중간 | 열림 | 협의회 테이블 FK 11건 마이그레이션 미작성 — BASKPM·BPOVWM·BRSLTM·BCMMTM·BEVALM·BCHKLM·BPLEVM·BPERFM·BMQNAM·BPQNAM·BSCHDM → BASCTM(`IT_PTL_ASCT_ID`). 설계·고아행 검사(2026-07 기준 0건)는 끝났고 `it_database/migrations`에 `REFERENCES` 제약이 하나도 없음 | 고아행 재검사 → `V{날짜}__AddCouncilForeignKeys.sql` 작성(`ON DELETE` 없음, 소프트 삭제 전제) → 로컬 Flyway 적용·협의회 전 흐름 재검증. `it_database` 범위라 착수 전 사용자 확인 | `prds/council-fk-erd.md` · `prds/PRD_c_20260709.md` §진행 중 |
| COUNCIL-015 | 중간 | 열림 | 협의회 알림 연동 미구현 — ① 사전 Q&A 등록 시 추진부서 담당자 알림, ② 평가위원 선정·일정 응답 요청 시 위원 알림("알림은 추후구현"). 협의회 도메인에서 알림을 발행하는 곳은 생략 판정 결재 결과(`CouncilSkipService`) 1곳뿐 | 공통 `common/notification` 아웃박스(`NotificationEvent`)로 발행 지점·수신자·문구 설계 → `QnaService.createQna`, `CommitteeService.saveCommittee`/일정 요청 흐름에 배선. 공통 알림 코드 수정이 필요하면 사용자 확인 | `prds/PRD_c_20260620.md`:5 · `prds/done/PRD_c_20260520.md`:320 |
| COUNCIL-016 | 중간 | 열림 | 계획협의회 평가 저장의 서버측 대상 사업 검증 부재 — `PlanEvaluationService.saveEvaluation`은 위원 자격·Y/N·사유만 검사하고 `abusMngNo`가 `plan-targets`(계획 스냅샷의 정보화사업)에 속하는지 대조하지 않음. 프론트 표출에만 의존 | 저장 시 대상 사업 집합과 대조해 범위 밖 사업은 400, 경상사업 제외 규칙도 서버에서 강제. 테스트 추가 | `prds/PLAN_c_20260709_impl.md`:81 |
