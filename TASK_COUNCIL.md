...
ID 최대 번호(활성·완료 합산): COUNCIL-058.

COUNCIL-053~055는 2026-09-27 결과서 전이 우회·평가 수정 취소·일정 선택 범위 문제로 등록하고, 구현 및 대상 회귀 검증 후 완료 이력으로 옮겼습니다.
COUNCIL-056~057은 2026-09-27 02 기준 재조사에서 찾은 계획평가 완료 시 결과서 단계 자동 전이 누락과 02 결과서 빈 확정 문제이며, 같은 날 구현·검증을 마쳐 완료 이력으로 옮겼습니다.
COUNCIL-058은 계획 상세 "협의회 신청" 버튼의 관리자 한정 노출과 기존 협의회 "열기" 분기이며, 2026-09-27 사용자 결정으로 구현·검증을 마쳐 완료 이력으로 옮겼습니다. 같은 결정의 "결재완료 계획만 허용"은 계획 결재 흐름이 없어 미구현이며 아래 범위 밖 발견에 있습니다.
...
- 계획(`TPRMPP_BPLANM`)에 결재 흐름이 없다(2026-09-27 발견). 진행상태 컬럼·결재 연동·상신 API·화면 상태 표시가 모두 없어 "결재완료 계획만 협의회 신청 허용"(COUNCIL-058 사용자 결정)을 판정할 원천이 없다. 계획 결재 흐름 신설은 협의회 밖 예산·계획 도메인 과제다.
