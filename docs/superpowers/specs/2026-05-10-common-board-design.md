...
- **작성일**: 2026-05-10
- **상태**: Draft (사용자 검토 대기 — v2: 메타 표준 컬럼명 적용)
- **대상 시스템**: IT Project Portal (it_backend / it_frontend)
- **참조 문서**: 루트 `CLAUDE.md`, 루트 `README.md` §11(메타 용어), `it_backend/CLAUDE.md`, `it_backend/docs/guides/data-model.md`
...
| # | 항목 | 결정 |
|---|---|---|
| 1 | 답변글 트리 표현 | **그룹+순서+레벨 3컬럼** (`NAC_GRP_NO`/`NAC_GRP_SQN`/`NAC_GRP_LV`) + `PRLM_NAC_MNG_NO` 보존 |
...
| 6 | 댓글 기능 | **답변글(`REP`) + 댓글(`CMT`) 둘 다 유지**. 댓글도 무제한 트리(`CMT_GRP_*` 3컬럼). 1차 첨부 미지원 |
...
| 게시판 댓글 테이블 | `TAAABB_CCMTM` |
| 게시판 댓글 엔티티 | `Ccmtm` |
| 게시판 댓글 변경 로그 테이블 | `TAAABB_CCMTL` |
| 게시판 댓글 변경 로그 엔티티 | `CcmtmL` |
...
| 댓글 PK (`CMT_MNG_NO`) | `CMT-{YYYY}-{0001}` | `CMT-2026-0001` |

#### 3.1.4 약어 사전 매핑
- ✅ = README §11 메타 용어에 존재 (그대로 사용)
...
| --------- | ----------------------------------------------- | ------------------------------------------------------- |
| `MNG`     | 관리                                              | ✅                                                       |
| `NO`      | 번호                                              | ✅(BG, MNG_NO 등 결합 표준)                                   |
| `SNO`     | 일련번호                                            | 🔧 (`BG_SNO`, `IT_MNGC_SNO`)                            |
| `NM`      | 명/이름                                            | 🔧 (`PRJ_NM`, `DOC_NM`)                                 |
| `TP`      | 유형                                              | 🔧 (`PRJ_TP`, `IVG_TP`)                                 |
| `CONE`    | 내용                                              | 🔧 (`DOC_CONE`, `IVG_CONE`)                             |
| `DTM`     | 일시                                              | ✅                                                       |
| `USR`     | 사용자                                             | ✅                                                       |
| `GRP`     | 그룹                                              | ✅                                                       |
| `BICE`    | 담당                                              | ✅                                                       |
| `KD`      | 종류 (카테고리)                                       | ✅                                                       |
| `RMK`     | 비고                                              | 🔧 (코드베이스 표준)                                           |
| `USE_YN`  | 사용여부                                            | 🔧 (코드 광범위 사용)                                          |
| `BBR`     | 담당부서                                            | 🔧 (CLAUDE.md §5.14, `BBR_C`/`BBR_NM`)                  |
| `FL`      | 파일                                              | 🔧 (`FL_MNG_NO`, `FL_DTT`)                              |
| `STA`     | 시작                                              | 🔧 (`STT_DT`, `MSM_STT_DT`) — 본 스펙은 PRD 원안 `STA_YMD` 보존 |
| `END`     | 종료                                              | ✅                                                       |
| `YMD`     | 년월일                                             | 🔧 (`PLN_YY` 류와 동급, 일자 표준)                              |
| `INQ`     | 조회                                              | 메타용어                                                    |
| `NBR`     | 횟수/번호(카운트)                                      | 메타용어                                                    |
| `BLB`     | 게시판 (Bulletin Board)                            | 메타용어                                                    |
| `REP`     | 답변 (Reply)                                      | 메타용어                                                    |
| `NEC`     | 필수 (Necessary)                                  | ➕                                                       |
| `PIN`     | 고정 (Pin)                                        | ➕                                                       |
| `ATH`     | 권한 (Authority)                                  | 메타용어                                                    |
| `ENR`     | 등록 (= 작성)                                       | 🔧 (`FST_ENR_USID`, `FST_ENR_DTM` 코드 표준)                |
| `LMT`     | 한정/제한 (Limit)                                   | ➕                                                       |
| `SRE`     | 화면 (Display)                                    | 메타용어                                                    |
| `SQN`     | 순서 (Order)                                      | 메타용어                                                    |
| `IPRT`    | 중요도 (Importance)                                | ➕                                                       |
| `APG`     | 첨부 (Appendage, PRD 원안)                          | 메타용어                                                    |
| `PRLM`    | 상위 (PReLiMinary, 코드 `PRLM_HRK_OGZ_C_CONE` 등 사용) | 🔧                                                      |
| `NAC`     | 게시물                                             | 메타용어                                                    |
| `CMT`     | 댓글 (Comment)                                    | ➕                                                       |

> ➕ 표시 약어는 본 스펙 승인 시 루트 `README.md` §11에 추가 등재한다 (§13 오픈이슈 참조).
...
| 댓글사용여부        | `CMT_USE_YN`      | `VARCHAR2(1)`   | NO   | `'N'`   | Y/N                         |
| 첨부필수여부        | `FL_NEC_YN`       | `VARCHAR2(1)`   | NO   | `'N'`   | Y/N                         |
| 상위고정사용여부      | `PRLM_PIN_USE_YN` | `VARCHAR2(1)`   | NO   | `'N'`   | Y/N                         |
...
| 담당부서한정사용여부    | `BBR_LMT_USE_YN`  | `VARCHAR2(1)`   | NO   | `'N'`   | Y면 본문 `BBR_C` 활성            |
| 담당부서한정코드      | `BBR_LMT_C`       | `VARCHAR2(8)`   | YES  | NULL    | 게시판 단위 부서 제한                |
...
| 중요도코드 | `IPRT_C` | `VARCHAR2(32)` | NO | `'IPRT_C_001'` | `001`=일반/`002`=중요/`003`=긴급 |
| 상위고정여부 | `PRLM_PIN_YN` | `VARCHAR2(1)` | NO | `'N'` | Y/N |
| 화면여부 | `SRE_YN` | `VARCHAR2(1)` | NO | `'Y'` | 노출 토글 (즉시 비공개) |
| 담당부서코드 | `BBR_C` | `VARCHAR2(8)` | YES | NULL | 공개 대상 부서. 메타 `BBR_LMT_USE_YN='Y'`일 때 (NULL=전체) |
| 시작일자 | `STA_YMD` | `DATE` | YES | NULL | 공개 시작일 (NULL=즉시) |
...
| 게시물그룹레벨 | `NAC_GRP_LV` | `NUMBER(2)` | NO | `0` | 0=원글, 1=답, 2=답의답… |
| 상위게시물관리번호 | `PRLM_NAC_MNG_NO` | `VARCHAR2(32)` | YES | NULL | 직계 부모 PK |
| **공통** | — | — | — | — | **`BaseEntity` 상속** |

### 3.4 `TAAABB_CCMTM` (게시판 댓글)

| 표준용어(Comment) | 컬럼명 | 타입 | NULL | 기본값 | 비고/예시 |
|---|---|---|---|---|---|
| 댓글관리번호 | `CMT_MNG_NO` | `VARCHAR2(32)` | NO | — | **PK**. `CMT-2026-0001` |
| 게시물관리번호 | `NAC_MNG_NO` | `VARCHAR2(32)` | NO | — | FK → `CBLBCM.NAC_MNG_NO` |
| 댓글내용 | `CMT_CONE` | `CLOB` | NO | — | 댓글 본문, sanitize 의무 |
| 화면여부 | `SRE_YN` | `VARCHAR2(1)` | NO | `'Y'` | 노출 토글 |
| 댓글그룹번호 | `CMT_GRP_NO` | `VARCHAR2(32)` | NO | — | 최상위 댓글 PK; 자기 자신이 최상위면 자기 PK |
| 댓글그룹순서 | `CMT_GRP_SQN` | `NUMBER(5)` | NO | `0` | 그룹 내 정렬 |
| 댓글그룹레벨 | `CMT_GRP_LV` | `NUMBER(2)` | NO | `0` | 0=원댓글, 1=대댓글, 2=대대댓글… |
| 상위댓글관리번호 | `PRLM_CMT_MNG_NO` | `VARCHAR2(32)` | YES | NULL | 직계 부모 댓글 PK |
| **공통** | — | — | — | — | **`BaseEntity` 상속** |

> 댓글은 게시판 메타의 `CMT_USE_YN='Y'`인 경우에만 활성화. 첨부파일은 1차 미지원 (§1.3 비-스코프).

### 3.5 `TAAABB_CBLBCL` / `TAAABB_CBLBML` / `TAAABB_CCMTL` (변경 로그)
`BgdocmL`/`BplanmL`과 동일 패턴. `Cblbmm`·`Cblbcm`·`Ccmtm` 각각에 `@LogTarget(entity = CblbmmL.class)` / `@LogTarget(entity = CblbcmL.class)` / `@LogTarget(entity = CcmtmL.class)` 부여 → `ChangeLogEntityListener`가 자동 적재.
...
CREATE INDEX IDX_CBLBCM_LIST
    ON TAAABB_CBLBCM (BLB_MNG_NO, DEL_YN, SRE_YN, PRLM_PIN_YN DESC, FST_ENR_DTM DESC);
...
-- 3. 직계 부모 탐색
CREATE INDEX IDX_CBLBCM_PRLM
    ON TAAABB_CBLBCM (PRLM_NAC_MNG_NO);
...
-- 7. 댓글 목록 (게시물별)
CREATE INDEX IDX_CCMTM_LIST
    ON TAAABB_CCMTM (NAC_MNG_NO, DEL_YN, SRE_YN, FST_ENR_DTM);

-- 8. 댓글 트리 정렬
CREATE INDEX IDX_CCMTM_GRP
    ON TAAABB_CCMTM (CMT_GRP_NO, CMT_GRP_SQN);

-- 9. 직계 부모 댓글 탐색
CREATE INDEX IDX_CCMTM_PRLM
    ON TAAABB_CCMTM (PRLM_CMT_MNG_NO);
```
...
-- 2) 공통코드: 중요도 (IPRT_C)
INSERT INTO TAAABB_CCODEM VALUES ('IPRT_C', 'IPRT_C_001', '일반', 1, 'Y', ...);
INSERT INTO TAAABB_CCODEM VALUES ('IPRT_C', 'IPRT_C_002', '중요', 2, 'Y', ...);
INSERT INTO TAAABB_CCODEM VALUES ('IPRT_C', 'IPRT_C_003', '긴급', 3, 'Y', ...);
...
-- 4) 게시판 메타 시드: 공지사항 (관리자 등록, 답변X, 댓글X, 상위고정 사용)
INSERT INTO TAAABB_CBLBMM (
    BLB_MNG_NO, BLB_NM, BLB_TP,
    REP_USE_YN, CMT_USE_YN, FL_NEC_YN, PRLM_PIN_USE_YN, KD_USE_YN,
    INQ_ATH_C, ENR_ATH_C, BBR_LMT_USE_YN,
...
-- 5) 게시판 메타 시드: 자료실 (전체 등록, 답변X, 댓글O, 첨부 필수, 카테고리)
INSERT INTO TAAABB_CBLBMM (
    BLB_MNG_NO, BLB_NM, BLB_TP,
    REP_USE_YN, CMT_USE_YN, FL_NEC_YN, PRLM_PIN_USE_YN, KD_USE_YN,
    INQ_ATH_C, ENR_ATH_C, BBR_LMT_USE_YN,
...
> 답변글(`NAC_GRP_*`)과 댓글(`CMT_GRP_*`)은 **동일한 그룹+순서+레벨 3컬럼 패턴**을 사용한다. 아래는 답변글 기준 표기이며, 댓글은 `NAC_*` → `CMT_*`, `PRLM_NAC_MNG_NO` → `PRLM_CMT_MNG_NO`로 치환하여 그대로 적용한다.
...
NAC_GRP_SQN      := 0
NAC_GRP_LV       := 0
PRLM_NAC_MNG_NO  := NULL
```

### 4.2 답변글 등록
부모 글의 `(NAC_GRP_NO, NAC_GRP_SQN, NAC_GRP_LV)` 조회 후:
```
...
2. 새 답변:
   ...
   NAC_GRP_LV       := 부모.NAC_GRP_LV + 1
   PRLM_NAC_MNG_NO  := 부모.NAC_MNG_NO
```
...
SELECT *
 ...
 WHERE BLB_MNG_NO = :blbMngNo
   AND DEL_YN = 'N'
   AND SRE_YN = 'Y'
   AND (STA_YMD IS NULL OR STA_YMD <= TRUNC(SYSDATE))
   AND (END_YMD IS NULL OR END_YMD >= TRUNC(SYSDATE))
 ORDER BY PRLM_PIN_YN DESC,
          NAC_GRP_NO DESC,    -- 그룹 단위 묶음, 최신 그룹 위
          NAC_GRP_SQN ASC;    -- 그룹 내 트리 순서
```

> SQN 재정렬은 단일 트랜잭션 + `NAC_GRP_NO`(또는 `CMT_GRP_NO`) 행 단위 비관적 락 (`SELECT ... FOR UPDATE`).
...
SELECT *
 ...
 WHERE NAC_MNG_NO = :nacMngNo
   AND DEL_YN = 'N'
   AND SRE_YN = 'Y'
 ORDER BY CMT_GRP_NO ASC,    -- 댓글 그룹은 작성 순서대로 (오래된 것 위)
          CMT_GRP_SQN ASC;   -- 그룹 내 트리 순서
```

> 댓글은 게시물과 달리 `CMT_GRP_NO ASC`(오래된 댓글이 위)가 일반적인 UX. 게시판 메타에서 정렬 방향을 옵션화할 수도 있으나 1차 단순 적용.
...
canReadBoard(user, board) :=
    ...
    OR (
        ...
        AND (board.BBR_LMT_C IS NULL OR user.bbrC = board.BBR_LMT_C)
    )
```
...
canReadPost(user, post, board) :=
    ...
    AND (
        ...
        OR (
            post.SRE_YN = 'Y'
            AND (post.STA_YMD IS NULL OR post.STA_YMD <= TODAY)
            AND (post.END_YMD IS NULL OR post.END_YMD >= TODAY)
            AND (
                board.BBR_LMT_USE_YN = 'N'
...
> **댓글 첨부 1차 미지원**: 댓글(`CCMTM`)에는 `Cfilem` 연결을 두지 않는다. 짧은 텍스트 본문만 허용. 향후 요구 발생 시 `ORC_DTT="공통게시판댓글"` 별도 값으로 확장 가능.
...
- [ ] `CMT_CONE` 길이 검증 (1차: 2,000자 한도 권장 — §13 오픈이슈)
- [ ] `CMT_CONE`도 `HtmlSanitizer.sanitize()` 통과 (단순 텍스트라도 XSS 방지)
...
| `POST` | `/api/boards/{blbMngNo}/posts/{nacMngNo}/pin` |
...
| `POST` | `/api/boards/{blbMngNo}/posts/{nacMngNo}/comments/{cmtMngNo}/replies` | 대댓글 |
| `PUT` | `/api/boards/{blbMngNo}/posts/{nacMngNo}/comments/{cmtMngNo}` | 수정 |
| `DELETE` | `/api/boards/{blbMngNo}/posts/{nacMngNo}/comments/{cmtMngNo}` | Soft Delete |
...
- `kdC` — 카테고리 (종류코드)
- `iprtC` — 중요도
- `staYmdFrom`, `staYmdTo` — 등록일 범위
- `bbrC` — 부서 필터 (CLAUDE.md §5.14 표준 패턴)
- 페이징: `page`, `size`, `sort`
- 기본 정렬: `(PRLM_PIN_YN DESC, FST_ENR_DTM DESC)` — 상위고정 우선, 그다음 최신순. 답변글 그룹 보존 시 §4.3 정렬과 함께 사용.
...
`Cblbmm`·`Cblbcm`·`Ccmtm` 각각에 `@LogTarget` 부여. `ChangeLogEntityListener`가 자동 INSERT/UPDATE/DELETE 감지 후 `TAAABB_CBLBML`/`TAAABB_CBLBCL`/`TAAABB_CCMTL`에 적재.
...
5. 첨부파일 다운로드 카운트 컬럼 (`FL_DWNL_NBR`) — 자료실 인기 자료 통계
6. 댓글 첨부파일 지원 (요구 발생 시 — `ORC_DTT="공통게시판댓글"` 추가)
7. **README §11 메타 용어 사전에 신규 약어 5종 등재**: `NEC`(필수)/`PIN`(고정)/`LMT`(한정)/`IPRT`(중요도)/`CMT`(댓글)
...
- **공지사항 정책**: 시드 `REP_USE_YN='N'` + `CMT_USE_YN='N'` (관리자 일방 안내) 정책 확인 필요
- **자료실 정책**: 시드 `REP_USE_YN='N'` + `CMT_USE_YN='Y'` + `FL_NEC_YN='Y'` 정책 확인 필요
- **삭제된 부모 댓글**: 자식 댓글이 있는 댓글 삭제 시 "삭제된 댓글입니다" 표시 + 자식 보존 (현 설계). 자식까지 cascade 삭제 정책 검토 여부
- **신규 약어 등재**: §3.1.4 ➕ 표시 약어 5종(`NEC`/`PIN`/`LMT`/`IPRT`/`CMT`)을 README §11에 추가 등재 필요
