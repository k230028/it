...
## 4.2 현재 DB 데이터 상태 — 실측 (2026-06-06) 및 처리 결정

시스템은 이전 마이그레이션(V20260528/V20260604/V20260605)으로 **이미 부분 마이그레이션된 상태**다.
컬럼 물리명은 신규명으로 리네임 완료, 일부 값은 선행 백필됨. 실측 분포와 처리 결정:

| 컬럼 | 실측 | 처리 결정 |
| --- | --- | --- |
| `Bprojm.ABUS_TC` | `'01'` 20건 (이미 목표값) | **유지**(백필 불요) |
| `Bcostm.BG_UNT_ABUS_C` | `'501'` 19건, NULL 2건 | **유지**(이미 목표값), NULL은 그대로 |
| `Capplm.APF_PRG_STS_C` | `'02'` 등 2자리 | 1자리(`1~4`)로 백필 |
| `Bcostm.DFR_CLE_C` | `'0'` 19건, NULL 2건 | **`'0'→'1'`(월)** 로 백필 |
| `Btermm.DFR_CLE_C` | `'0'` 115건 | **`'0'→'1'`(월)** 로 백필 |
| `Btermm.IT_PTL_TMN_SVC_TC` | 전체 NULL 115건 | **`001~003` 랜덤 채움** |
| `Btermm.IT_PTL_TMN_KD_TC` | 한글값 97/18건 (이상) | **`001~003` 랜덤 재설정** |

**핵심 결정**:
- **백필 멱등성**: 이미 목표값인 행은 건드리지 않는다(WHERE 절로 구값/이상값만 필터). 재실행 안전.
- CCODEM은 아직 **구 그룹ID** 보유 → 그룹ID 리네임(Phase 1) + BE/FE 리터럴 교체(Phase 3/4)는 **단일 빅뱅 배포로 동시** 반영(중간 상태에서 조회 실패 방지).
- `Btermm` 단말 데이터는 더티 데이터로 간주, 정확값 복원이 아니라 **유효 코드값(001~003) 랜덤 채움**으로 정합성만 확보.
