...
| 그 밖의 사용자 | `04` | 위 조건에 더해 대상 사업 소요자원에 정보보호 항목(`BITEMM.SECT_SYS_UTZ_YN='Y'`, 최종본)이 있음 |

두 역할을 모두 가진 사용자는 두 행의 합집합이다. `02`는 사업이 아니라 계획을 대상으로 하므로 시스템관리자만 신청한다. 대상 사업이 없거나 삭제된 경우는 기존 흐름의 예외를 유지한다.
