...
- 국내점포는 공통코드 그룹 `IT_PTL_APF_DCR_PT_C`(결재자직위코드)로 판정한다. `CDVA_ID`는 직위코드 `PT_C` 값, `CDVA_NM`은 직위명, `CO_CDVA_NM`은 차수(`1` 또는 `2`)다.
  - 1차: 기안자와 같은 팀코드(`TEM_C`)와 같은 부점코드(`BBR_C`)인 재직자 중 차수 `1` 직위(팀장, CO)
  - 2차: 기안자와 같은 부점코드인 재직자 중 차수 `2` 직위(부장, 실장, 지점장, 국장, 센터장, 사장)
...
직위코드 실제 값은 운영 `TPRMPP_CUSERI.PT_C`에서 확인해 시드 마이그레이션에 넣는다. 설계는 값이 바뀌어도 코드 수정 없이 공통코드로 조정할 수 있게 한다. 구현 계획 시작 전에 값을 확보한다.
