...
- **설계**:
  - **스키마(Flyway)**: `TPRMPP_CRTOKM`에 `FAM_NM`(가족명=토큰패밀리, 로그인 1회=1패밀리, VARCHAR2(100)) + `AVL_YN`(유효여부, VARCHAR2(1): Y=활성 토큰 / N=회전된 구토큰) 컬럼 추가. `it_database/migrations/V*.sql` 신규(체크섬 추적 — 기존 스크립트 수정 금지).
  - **회전 로직 재작업**: 로그인 시 새 `FAM_NM` 부여(`AVL_YN='Y'`). 회전 시 구 토큰을 삭제 대신 **`AVL_YN='N'`(회전됨) 표식 유지**, 신규 토큰을 동일 `FAM_NM`·`AVL_YN='Y'`로 저장. refresh 시:
    - 제출 토큰 `AVL_YN='Y'`(활성) → 정상 회전.
    - 제출 토큰 `AVL_YN='N'`(이미 회전됨) → **재사용 탐지 → 해당 패밀리(`FAM_NM`/eno) 전체 폐기**(로그 경고). 공격자·정상 사용자 모두 재인증 요구.
    - 미발견/만료 → 기존 처리.
