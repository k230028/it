...
**1-1의 선택 항목 8개는 셀이 아니라 양식 컨트롤 체크박스다.** `업무구분`·`사업유형`·`디지털 기술 유형`·`주 사용자`·`중복 여부`·`법규상 완료시기`·`최종보고`·`추진가능성`은 셀이 비어 있고 시트 위에 뜬 도형으로 표시된다. `FormCheckboxReader`가 형식별로 (위치·문구·체크상태)를 꺼낸다 — `.xls`는 Escher `ObjRecord`의 `FtCblsData`(sid `0x12`) 첫 2바이트, `.xlsx`는 시트 XML의 `control` 앵커 + `ctrlProps`의 `checked` + VML 텍스트박스의 문구다. `.xlsx`에서 `CTWorksheet.getControls()`는 `mc:AlternateContent` 때문에 **null을 돌려주므로** 파트 XML을 직접 읽는다.

선택 규칙은 `CheckboxFieldReader`로 분리해 단위 테스트한다(POI로는 양식 컨트롤이 든 워크북을 만들 수 없어 픽스처가 불가능하다). 범위는 라벨 열부터 **같은 행 다음 라벨 직전까지** — 한 행에 항목이 둘 놓이는 배치(`중복 여부 … 법규상 완료시기 …`)가 있다. 복수 선택은 쉼표로 잇고, 코드 컬럼(최종보고·추진가능성)은 문구를 `FormLexicon.canonicalOptionName`으로 코드값명에 맞춘 뒤 **첫 매칭**을 쓴다(추진가능성 칸에 보조 `Y/N` 체크박스가 끼어 있다).

`법규상 완료시기`만 값을 반입하지 않는다. 컬럼(`FLF_FSG_DT`)은 YYYYMMDD 의무완료기한인데 양식은 `2026년 이내`처럼 구간을 고르게 되어 있어 날짜로 환산할 근거가 없다. 추정 날짜를 법규 기한 칸에 남기지 않고 안내만 낸다. `별도없음`은 미기재가 아니라 확정된 답이므로 안내도 내지 않는다.

**1-2 소요자원 표의 비목(중분류)은 C열 고정이다.** 이 시트는 `A 소요예산(블록 라벨) | B 대분류 | C 중분류 | D 항목`의 고정 배치라 열을 직접 짚는다. 시트 ②는 블록 라벨 열이 없어 한 칸 당겨지므로(실측: `A 구분 | B 중분류 | C 항목`) 항목 열 바로 왼쪽으로 잡는다. 두 시트가 같은 리더를 쓰지만 진입점을 나눠(`readCapitalResource`·`readRecurring`) 각자의 규칙을 못박는다 — 항목 열 기준으로 한 칸 왼쪽을 잡는 공통 규칙은 부점이 열을 하나 끼워 넣은 파일에서 대분류나 빈 열을 비목으로 읽어 조용히 어긋난다.

셀 읽기에서 실측으로 확인된 함정 둘은 `SheetAnchorScanner`가 흡수한다. ① **수식 셀은 캐시된 계산 결과를 쓴다.** `DataFormatter.formatCellValue(cell)`에 평가기를 주지 않으면 수식 원문(`SUM(E10*F10)`)이 나오고, 숫자 파싱이 실패해 그 행이 **진단 한 줄 없이** 사라진다(런던 시트 ②의 소요자원 7행 전량). 평가기로 재계산하지는 않는다 — 깨진 참조 하나로 파일 전체를 잃는다. ② **라벨 칸의 병합 영역을 건너뛰고 값을 찾는다.** 실 제출본은 라벨을 A:B로 병합하는데, 병합 안쪽 셀은 좌상단(=라벨) 값을 돌려주므로 라벨 바로 오른쪽을 값으로 읽으면 라벨 자신이 나온다. 그 값을 "다음 라벨"로 판정해 탐색을 접으면 1-1의 항목이 통째로 비고 사업이 하나도 생기지 않는다.

### 3.3 부서 귀속

시트 ②·③에는 부서 열이 아예 없다. 1-1만 `주관부문/주관부서/팀`을 담는다. 파일명도 신뢰할 수 없다(샘플 3건 중 1건은 괄호 안이 부점명이 아니라 사업명이다).

**`webkitRelativePath`의 폴더명을 부서로 삼는다.** `OrgIdentityResolver.resolveOrgFolder()`로 해석하며, 미해석·중의적이면 그 폴더의 모든 파일이 `ORG_UNRESOLVED` BLOCKER가 되고 미리보기에서 한 번 고르면 폴더 전체에 적용된다.

**어느 폴더가 부서인지는 최상위가 아니라 `부서명(부서코드)` 표기로 정한다.** 관리자가 `2026\IT기획부(180)\...`처럼 연도 폴더를 통째로 고르는 운용이 실제로 나와서(실측), 최상위를 부서로 삼으면 모든 파일이 `2026`으로 묶여 전량 차단된다. 파일이 든 폴더부터 위로 훑어 표기를 만족하는 **가장 안쪽** 폴더를 쓰고, 그런 폴더가 없으면 종전대로 최상위를 쓴다(`런던지점\2026\붙임.xls`처럼 부서 아래 분류 폴더가 오는 구조 때문에 바로 위 폴더로 대체하지 않는다).

폴더는 `자금운용실(420)`처럼 **`부서명(부서코드)`로 만들고, 병기된 코드를 이름보다 먼저 기준으로 삼는다.** 이름 매칭은 상·하위 조직이 함께 걸려 중의적으로 차단되거나(`금융공학실`↔`금융공학실 퀀트인프라팀`) 조직 개편으로 부점명이 바뀌면 통째로 미해석이 되지만, 코드는 흔들리지 않는다. 괄호 안은 영숫자일 때만 코드로 읽어 `요청서(최종)` 같은 한글 메모를 코드 자리로 오인하지 않는다. 코드가 없거나 조직에 없는 코드면 괄호를 뗀 이름으로 종전 3단계 해석을 그대로 탄다.

**주관부서와 주관부문/본부는 폴더 부서코드가 유일한 근거다.** 1-1 시트의 `주관부서/팀` 기재값으로 조직을 찾는 경로는 없앴다 — 조직 개편으로 부점명이 낡거나(`IT인프라팀`) 상·하위 조직이 함께 걸려 중의적이 되면 사업 전체가 차단되는데, 부서는 폴더에 코드로 이미 확정돼 있다. 부문/본부는 그 부서의 상위조직명(`CORGNI.PRLM_HRK_OGZ_C_CONE`)에서 끌어온다.

**비목의 국내·국외는 부서코드로 가른다.** 부서코드가 `9`로 시작하면 국외 점포다(실측: 런던지점 `920`). 통화로 가르지 않는 이유는 통화가 부점 소속을 말해 주지 않기 때문이다 — 국내 부점도 외화 계약을 맺고, 국외 점포도 원화로 적어 내는 행이 있다. 비목의 국내·국외는 돈의 단위가 아니라 부점의 소속으로 정해진다.

**경상사업(②)·일반관리비(③)의 주관팀장은 `(확인자)`, 담당자는 `(작성자)`다.** 두 시트에는 1-1의 `관련 조직` 블록이 없고 시트 맨 위에 한 번 적는다. 실 제출본이 두 표기를 섞어 쓴다 — 라벨과 이름이 다른 칸인 형태(②)와 같은 칸에 붙은 형태(`(작성자) Luke Buckingham-Brown 과장`, ③). `BCOSTM`에는 팀장 컬럼이 없어 일반관리비는 담당자(`CGPR_ID`)만 담는다.

**적혀 있지 않으면 비워 둔다.** 예전에는 일반관리비 담당자를 업로드 사용자로 채웠는데, 그러면 원장에 사실이 아닌 담당자가 남는다. 1-1도 같은 규칙이며, 그쪽은 `관련 조직` 블록이 정본이라 머리말의 확인자·작성자를 쓰지 않는다(실측 파일에서 두 곳이 어긋났다).

**주관팀은 이름만 담는다.** 팀코드(`SVN_TEM_C`)는 `CORGNI`에 없어 이름으로 찾을 수 없으므로, `주관부서/팀`의 `/` 뒤 문구를 팀명 컬럼(`SVN_TEM_NM`)에 그대로 보관한다. `ProjectDto.CreateRequest.svnTemNm`은 이 경로를 위한 대체값이며, 팀코드나 담당자 소속으로 팀명이 잡히면 그쪽이 우선이다.

**담당자는 사번으로 해석하지 않고 이름만 담는다.** `팀장`·`IT팀장`은 기재된 이름, `실무자(정/부)`·`IT실무자(정/부)`는 `/` 앞의 <b>정</b>만 담는다. 담당자 컬럼(`USID`·`TLR_USID` 등)이 사번과 이름을 모두 받는 자리이고, 부점이 적어 내는 이름은 인사 표기와 어긋나거나 동명이인이라 사번 확정이 자주 실패한다 — 사람 이름 하나 때문에 사업 전체를 막지 않는다. 사번은 반입 후 상세 화면에서 맞춘다. 컬럼이 14자라 영문 성명은 잘라 담고 경고를 남긴다.
...
| 코드 | severity | 조건 |
| --- | --- | --- |
**진단마다 해소에 필요한 입력을 서버가 지정한다(`decision`).** 화면의 `결정` 열은 이 값으로 위젯을 고른다 — `SELECT`(후보 선택)·`TEXT`·`DATE`(YYYYMMDD)·`AMOUNT_UNIT`(파일 단위 금액 배수)·`NONE`. 화면이 진단 코드나 필드명을 보고 위젯을 추측하지 않는 이유는, 서버가 읽지 않는 값을 입력받으면 사용자가 고쳤다고 믿은 채 같은 진단을 다시 보기 때문이다. **`NONE`이 아닌 종류를 지정했다면 같은 이름의 보정값을 서버가 실제로 읽어 반영해야 한다.**

선택 항목 8개의 보정값은 **그대로 저장값**이다(코드값명 저장 컬럼은 코드값명, 코드 저장 컬럼은 코드). 후보는 미기재 안내에 붙여 미리보기에서 바로 고르게 하며, 비목처럼 좁히지 못한 경우에는 **전체 목록을 후보로 낸다** — "골라 주세요"라고 하면서 후보를 비워 두면 드롭다운이 그려지지 않아 손댈 방법이 없다. `NONE`은 원본 파일을 고쳐야 하는 것(1-1↔1-2 합계 불일치)뿐이다.

**같은 사건을 두 번 보고하지 않는다.** 비목이 비어 있는 이유는 어댑터가 해석하지 못했기 때문이고 어댑터는 그 사실을 행 좌표·후보와 함께 이미 보고했으므로, 검증기는 어댑터가 짚은 대상(품목명·계약명)의 비목 필수값 검사를 건너뛴다. 중복을 남기면 결정으로 풀 수 없는 차단이 하나 더 생겨, 행 진단에서 비목을 골라도 파일이 계속 막힌다(실측: 자금운용실 품목 3·4·5 때문에 결정 3건을 모두 내고도 반영 불가).

진단은 **무엇이 문제인지(`message`)와 어디인지(`sheet`·`excelRow`·`subject`)를 나눠 담는다.** 한 파일이 사업·품목·계약을 수십 건 만들어 같은 문구가 여러 줄 늘어서므로, 대상 이름 없이는 어느 줄을 고쳐야 하는지 알 수 없다(실측: `품목 비목코드가 비어 있습니다`가 구분 없이 3줄). 문구에 대상을 되풀이하지 않는다 — 화면이 좌표와 대상을 칩으로 따로 그려 두 번 나온다. 품목 대상은 `사업명 · 품목 N 품목명`으로 만들어 품목명이 빈 행도 순번으로 짚는다.
...
| 부서 귀속 | `부서명(부서코드)` 폴더의 코드가 유일한 근거. 팀은 이름만, 담당자도 이름만 |
