...
> **2026-08-22 개정 — PID 자리에 인스턴스ID (FE-55)**
>
> `FILE_LOG_PATTERN`을 그대로 재현할 수 없다. 기본 패턴은 PID를 포함하는데, 다운로드는 피어
> 인스턴스의 링버퍼를 위임 조회한 결과를 담을 수 있고 `WasLogEntry`에는 그 피어의 PID가 없다.
> 이 서버의 PID를 적으면 남의 로그에 이 인스턴스의 PID를 붙이는 셈이라 파서를 속인다.
>
> 따라서 **자리와 구분자는 그대로 두고 PID 자리에만 인스턴스ID를 넣는다.** 다중 인스턴스
> 운영에서는 어느 서버의 로그인지가 PID보다 유용하므로, 이 변형은 §5.6의 취지(같은 도구로 열기)를
> 해치지 않는다. 형식이 다르다는 사실은 **파일 첫 줄에 `#` 주석**으로 밝힌다.
>
> 로거 칸은 logback의 `TargetLengthBasedClassNameAbbreviator`를 그대로 써서
> `%-40.40logger{39}`와 같은 문자열을 낸다 — 직접 구현하면 39자 안에 드는 이름을 불필요하게
> 줄이는 등 파일 로그와 어긋난다(실제로 구현 중 그 차이를 테스트가 잡았다).
...
> **2026-08-22 개정 — 수용 리스크 철회, 마스킹 도입 (BE-55)**
>
> 종전에는 마스킹을 적용하지 않고 리스크를 수용했다. 사용자가 도입을 결정해 §5.2의 적재 직전
> 지점(`RingBufferAppender.append`)에 `WasLogMasker`를 끼웠다.

**가리는 것은 둘뿐이다** — JWT·`Bearer`/`Basic` 토큰 값, 그리고 주민등록번호.
**사번은 가리지 않는다**: 어느 사용자의 요청에서 난 오류인지가 장애 추적의 출발점이라, 이것까지
가리면 이 화면의 쓸모가 사라진다.

**파일 로그의 원문은 그대로다.** 마스킹은 링버퍼에 담기는 사본에만 적용된다. 화면·다운로드로
나가는 경로만 가리는 것이 목적이고, 서버에 남는 로그로 조사하는 경로는 막지 않는다.

**적재 경로 비용**: 모든 로그 이벤트가 이 자리를 지나므로, 정규식을 돌리기 전에 값싼 사전
검사(`.` 포함 여부, `bearer`/`basic` 포함 여부, 숫자 6자 연속 여부)로 대부분의 줄을 건너뛴다.
가릴 것이 없으면 새 문자열을 만들지 않고 원본 인스턴스를 그대로 돌려준다.

**남는 리스크**: 위 두 형태를 벗어난 개인정보(예: 로그에 그대로 찍힌 이름·연락처)는 여전히
노출된다. 통제 수단은 ADMIN 권한과 §4.4의 감사 로그다.
