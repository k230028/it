...
- "헤더는 최상위 전용"이라는 제약은 `HED`와 함께 사라진다. `GRP`는 루트와 하위 어디에도 놓일 수 있으므로, 기존 헤더를 드래그&드롭으로 다른 메뉴 아래로 옮기는 것이 허용된다. 헤더와 그룹을 한 유형으로 합친 결과이며, 루트에 `GRP`만 허용한다는 규칙만 남는다.
- 유형·계층 검증은 생성·수정·이동 세 경로 모두에 적용한다. 수정은 대상 메뉴의 현재 상위 메뉴를 기준으로 계층 규칙을 다시 검증한다. 현재 `update`는 계층 검증을 호출하지 않아 루트 메뉴의 유형만 바꿔 규칙을 우회할 수 있다.
...
- `validateHierarchy`의 `"HED".equals(mnuTpC)` 판정을 계층 위치 기준으로 바꾼다. 루트(`hrkMnuId == null`)이면 `GRP`여야 한다는 규칙 하나만 남긴다. 유형과 위치가 이중으로 갖던 진실이 하나로 합쳐지고, "헤더는 하위로 이동할 수 없다"는 제약은 사라진다.
- `update`에 `validateHierarchy(req.getMnuTpC(), menu.getHrkMnuId())`를 추가한다. 현재 수정 경로에는 계층 검증이 없어 루트 메뉴의 유형만 `PGE`로 바꿔 규칙을 우회할 수 있다.
...
- `AdminMenuService`: `PGE`의 화면경로 필수·카탈로그 존재 검증, `GRP`의 화면경로 금지, `LNK`의 400, 루트 생성 시 `GRP`가 아니면 거부, `move`로 `PGE`를 루트에 올리면 거부, `GRP`를 다른 메뉴 하위로 이동하면 허용, `update`로 루트 메뉴 유형을 `PGE`로 바꾸면 거부.
