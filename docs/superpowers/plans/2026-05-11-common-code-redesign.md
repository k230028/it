...
**Goal:** `TAAABB_CCODEM`의 PK 구조를 `(C_ID, CDVA, STT_DT)`로 재정의하고, 카테고리/값을 분리해 다운스트림 FK가 CDVA만 저장하도록 변환. Dash/Underbar를 `_`로 통일. 단일 PR · 단일 점검창에서 빅뱅 전환.
...
COMMENT ON COLUMN TAAABB_CCODEM_V2.C_ID     IS '코드ID(컬럼/엔티티명)';
COMMENT ON COLUMN TAAABB_CCODEM_V2.CDVA     IS '코드값';
COMMENT ON COLUMN TAAABB_CCODEM_V2.STT_DT   IS '시작일자';
COMMENT ON COLUMN TAAABB_CCODEM_V2.END_DT   IS '종료일자';
COMMENT ON COLUMN TAAABB_CCODEM_V2.C_NM     IS '코드명';
COMMENT ON COLUMN TAAABB_CCODEM_V2.C_DES    IS '코드설명';
COMMENT ON COLUMN TAAABB_CCODEM_V2.CDVA_DTL IS '코드값상세';
COMMENT ON COLUMN TAAABB_CCODEM_V2.C_TP     IS '코드타입';
COMMENT ON COLUMN TAAABB_CCODEM_V2.C_TP_DES IS '코드타입설명';
COMMENT ON COLUMN TAAABB_CCODEM_V2.HRK_C    IS '상위코드({C_ID}_{CDVA})';
...
COMMENT ON TABLE TAAABB_CCODEL_V2 IS '공통코드 변경 로그(V2)';
```
...
```
TAAABB_BPROJM:  PRJ_TP, BZ_DTT, PUL_DTT, CUR, TCHN_TP, MN_USR, RPR_STS, PRJ_PUL_PTT
TAAABB_BPROJML: 동일
TAAABB_BRDOCM:  BZ_DTT
TAAABB_BRDOCML: BZ_DTT
TAAABB_BCOSTM:  DFR_CLE, IOE_CAT
TAAABB_BCOSTML: 동일
TAAABB_BITEMM:  IOE_CAT, DFR_CLE (사용 시)
```
...
UPDATE TAAABB_BPROJM SET PRJ_TP      = REGEXP_SUBSTR(REPLACE(PRJ_TP,     '-','_'), '[A-Z0-9]+$') WHERE PRJ_TP      IS NOT NULL;
UPDATE TAAABB_BPROJM SET BZ_DTT      = REGEXP_SUBSTR(REPLACE(BZ_DTT,     '-','_'), '[A-Z0-9]+$') WHERE BZ_DTT      IS NOT NULL;
UPDATE TAAABB_BPROJM SET PUL_DTT     = REGEXP_SUBSTR(REPLACE(PUL_DTT,    '-','_'), '[A-Z0-9]+$') WHERE PUL_DTT     IS NOT NULL;
UPDATE TAAABB_BPROJM SET CUR         = REGEXP_SUBSTR(REPLACE(CUR,        '-','_'), '[A-Z0-9]+$') WHERE CUR         IS NOT NULL;
UPDATE TAAABB_BPROJM SET TCHN_TP     = REGEXP_SUBSTR(REPLACE(TCHN_TP,    '-','_'), '[A-Z0-9]+$') WHERE TCHN_TP     IS NOT NULL;
UPDATE TAAABB_BPROJM SET MN_USR      = REGEXP_SUBSTR(REPLACE(MN_USR,     '-','_'), '[A-Z0-9]+$') WHERE MN_USR      IS NOT NULL;
UPDATE TAAABB_BPROJM SET RPR_STS     = REGEXP_SUBSTR(REPLACE(RPR_STS,    '-','_'), '[A-Z0-9]+$') WHERE RPR_STS     IS NOT NULL;
UPDATE TAAABB_BPROJM SET PRJ_PUL_PTT = REGEXP_SUBSTR(REPLACE(PRJ_PUL_PTT,'-','_'), '[A-Z0-9]+$') WHERE PRJ_PUL_PTT IS NOT NULL;

UPDATE TAAABB_BPROJML SET PRJ_TP      = REGEXP_SUBSTR(REPLACE(PRJ_TP,     '-','_'), '[A-Z0-9]+$') WHERE PRJ_TP      IS NOT NULL;
UPDATE TAAABB_BPROJML SET BZ_DTT      = REGEXP_SUBSTR(REPLACE(BZ_DTT,     '-','_'), '[A-Z0-9]+$') WHERE BZ_DTT      IS NOT NULL;
UPDATE TAAABB_BPROJML SET PUL_DTT     = REGEXP_SUBSTR(REPLACE(PUL_DTT,    '-','_'), '[A-Z0-9]+$') WHERE PUL_DTT     IS NOT NULL;
UPDATE TAAABB_BPROJML SET CUR         = REGEXP_SUBSTR(REPLACE(CUR,        '-','_'), '[A-Z0-9]+$') WHERE CUR         IS NOT NULL;
UPDATE TAAABB_BPROJML SET TCHN_TP     = REGEXP_SUBSTR(REPLACE(TCHN_TP,    '-','_'), '[A-Z0-9]+$') WHERE TCHN_TP     IS NOT NULL;
UPDATE TAAABB_BPROJML SET MN_USR      = REGEXP_SUBSTR(REPLACE(MN_USR,     '-','_'), '[A-Z0-9]+$') WHERE MN_USR      IS NOT NULL;
UPDATE TAAABB_BPROJML SET RPR_STS     = REGEXP_SUBSTR(REPLACE(RPR_STS,    '-','_'), '[A-Z0-9]+$') WHERE RPR_STS     IS NOT NULL;
UPDATE TAAABB_BPROJML SET PRJ_PUL_PTT = REGEXP_SUBSTR(REPLACE(PRJ_PUL_PTT,'-','_'), '[A-Z0-9]+$') WHERE PRJ_PUL_PTT IS NOT NULL;

UPDATE TAAABB_BRDOCM  SET BZ_DTT = REGEXP_SUBSTR(REPLACE(BZ_DTT,'-','_'), '[A-Z0-9]+$') WHERE BZ_DTT IS NOT NULL;
UPDATE TAAABB_BRDOCML SET BZ_DTT = REGEXP_SUBSTR(REPLACE(BZ_DTT,'-','_'), '[A-Z0-9]+$') WHERE BZ_DTT IS NOT NULL;

UPDATE TAAABB_BCOSTM  SET DFR_CLE = REGEXP_SUBSTR(REPLACE(DFR_CLE,'-','_'), '[A-Z0-9]+$') WHERE DFR_CLE IS NOT NULL;
UPDATE TAAABB_BCOSTM  SET IOE_CAT = REGEXP_SUBSTR(REPLACE(IOE_CAT,'-','_'), '[A-Z0-9]+$') WHERE IOE_CAT IS NOT NULL;
UPDATE TAAABB_BCOSTML SET DFR_CLE = REGEXP_SUBSTR(REPLACE(DFR_CLE,'-','_'), '[A-Z0-9]+$') WHERE DFR_CLE IS NOT NULL;
UPDATE TAAABB_BCOSTML SET IOE_CAT = REGEXP_SUBSTR(REPLACE(IOE_CAT,'-','_'), '[A-Z0-9]+$') WHERE IOE_CAT IS NOT NULL;
...
```sql
SELECT 'BPROJM.PRJ_TP'  col, COUNT(*) cnt FROM TAAABB_BPROJM WHERE PRJ_TP IS NOT NULL UNION ALL
SELECT 'BPROJM.BZ_DTT'  col, COUNT(*)     FROM TAAABB_BPROJM WHERE BZ_DTT IS NOT NULL UNION ALL
SELECT 'BPROJM.PUL_DTT' col, COUNT(*)     FROM TAAABB_BPROJM WHERE PUL_DTT IS NOT NULL UNION ALL
SELECT 'BPROJM.CUR'     col, COUNT(*)     FROM TAAABB_BPROJM WHERE CUR     IS NOT NULL UNION ALL
SELECT 'BCOSTM.DFR_CLE' col, COUNT(*)     FROM TAAABB_BCOSTM WHERE DFR_CLE IS NOT NULL UNION ALL
SELECT 'BCOSTM.IOE_CAT' col, COUNT(*)     FROM TAAABB_BCOSTM WHERE IOE_CAT IS NOT NULL;
-- (필요한 컬럼 추가)
```
...
ALTER TABLE TAAABB_CCODEM    RENAME TO TAAABB_CCODEM_OLD;
ALTER TABLE TAAABB_CCODEM_V2 RENAME TO TAAABB_CCODEM;
...
ALTER TABLE TAAABB_CCODEL    RENAME TO TAAABB_CCODEL_OLD;
ALTER TABLE TAAABB_CCODEL_V2 RENAME TO TAAABB_CCODEL;
```
...
ALTER TABLE TAAABB_CCODEM     RENAME TO TAAABB_CCODEM_FAILED;
ALTER TABLE TAAABB_CCODEM_OLD RENAME TO TAAABB_CCODEM;
ALTER TABLE TAAABB_CCODEL     RENAME TO TAAABB_CCODEL_FAILED;
ALTER TABLE TAAABB_CCODEL_OLD RENAME TO TAAABB_CCODEL;
-- 다운스트림 FK 데이터(V005 적용분)는 expdp 백업 복원으로 별도 복구.

-- D+30 정상 종료 후 정리:
-- DROP TABLE TAAABB_CCODEM_OLD;
-- DROP TABLE TAAABB_CCODEL_OLD;
-- DROP TABLE TAAABB_CCODEM_FAILED;
-- DROP TABLE TAAABB_CCODEL_FAILED;
```
...
@V20260512_006__rename_swap.sql
@V20260512_007__rename_swap_log.sql
SELECT C_ID, CDVA, STT_DT, C_NM, C_DES FROM TAAABB_CCODEM WHERE ROWNUM <= 5;
```
...
/**
 * 공통코드마스터.
 * 테이블: TAAABB_CCODEM, PK: (C_ID, CDVA, STT_DT)
 */
...
/**
 * 공통코드마스터(TAAABB_CCODEM) 변경 로그 엔티티.
 */
@Entity
@Table(name = "TAAABB_CCODEL", comment = "공통코드 변경 로그")
@Getter
@SuperBuilder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor
...
```md
## Summary
- TAAABB_CCODEM PK를 (C_ID, CDVA, STT_DT)로 확장
