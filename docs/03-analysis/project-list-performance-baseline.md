...
동일한 fixture와 3회 반복으로 재측정했다. 응답 bytes는 API 조회 크기를 바꾸지 않았으므로 변경 전과 동일하다.

| 화면 | 건수 | first card render 전 → 후 (ms) | DOM nodes 전 → 후 | 카드 전 → 후 | filter → next paint 전 → 후 (ms) |
| --- | ---: | ---: | ---: | ---: | ---: |
| 사업계획 | 100 | 970 → 894 | 2,833 → 837 | 100 → 20 | 32.7 → 16.8 |
| 사업계획 | 500 | 1,292 → 881 | 12,833 → 837 | 500 → 20 | 78.4 → 18.9 |
| 사업계획 | 1,000 | 1,575 → 906 | 25,344 → 837 | 1,000 → 20 | 442.5 → 21.2 |
| 소요예산 산정 | 100 | 1,005 → 880 | 2,847 → 840 | 100 → 20 | 32.6 → 21.0 |
| 소요예산 산정 | 500 | 1,270 → 884 | 12,847 → 840 | 500 → 20 | 85.2 → 22.5 |
| 소요예산 산정 | 1,000 | 1,648 → 902 | 25,347 → 840 | 1,000 → 20 | 411.1 → 21.9 |
| 정보화실무협의회 | 100 | 1,448 → 983 | 3,445 → 969 | 100 → 20 | 27.5 → 37.0 |
| 정보화실무협의회 | 500 | 1,526 → 937 | 15,845 → 969 | 500 → 20 | 60.0 → 28.2 |
| 정보화실무협의회 | 1,000 | 2,146 → 927 | 31,345 → 969 | 1,000 → 20 | 116.9 → 27.0 |

세 화면의 최초 카드 수는 20개이고, 1,000건 fixture의 필터 → 다음 paint 중앙값도 모두 100ms 이하이다. 이 수치는 모킹된 응답을 사용한 DOM 렌더링 결과이므로 API 전송 또는 서버 성능 개선으로 해석하지 않는다.
