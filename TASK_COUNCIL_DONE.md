...
| COUNCIL-052 | ✅ Done | 계획협의회(02)는 사전·주요 Q&A를 사용하지 않지만 신규 등록만 서버에서 막고 목록 조회와 기존 항목의 수정·답변·삭제 API는 열려 있었다. 과거·오입력 행이 남으면 화면을 우회해 계속 변경할 수 있고, 사전 Q&A 답변은 계획관리번호를 사업번호로 조회한 뒤 주관부서를 찾지 못해 비관리자 권한 검사까지 느슨해졌다. 두 Q&A 서비스의 모든 읽기·쓰기 진입점에서 02 유형을 거부하고, 사전 Q&A 답변은 검증한 협의회 원장을 권한 검사와 알림에 재사용한다 | [사전 Q&A](it_backend/src/main/java/com/kdb/it/domain/council/service/QnaService.java) · [주요 Q&A](it_backend/src/main/java/com/kdb/it/domain/council/service/MainQnaService.java) |
...
코드 변경(COUNCIL-052): `it_backend` — `QnaService.java`, `MainQnaService.java`, 단위 테스트 2개. 프론트·API 계약·DB 스키마 변경 없음.

검증 결과(COUNCIL-052): 수정 전 신규 회귀 테스트 2건이 의도대로 실패했고 수정 후 Q&A 서비스 테스트 30건, 협의회 도메인 전체 테스트, `spotlessJavaCheck`가 통과했다. Claude의 COUNCIL-051 프론트 라우팅·카드·겸직 접근 테스트도 별도로 3개 파일 33건 통과했다.

미검증 범위(COUNCIL-052): 실제 HTTP 호출과 과거 02 Q&A 데이터 존재 여부는 확인하지 않았다. 기존 데이터를 자동 삭제하지 않고 접근과 변경만 차단한다. 개발계 배포는 수행하지 않았다.
...
| COUNCIL-034 | ✅ Done | 정보보호관리자 목록 분기가 사업 기반 조회 결과만 걸러 계획협의회를 놓쳤다. 02 필수 위원 7팀에 정보보호기획(18301)이 있어 그 팀장이 ITPAD002면 자기 협의회가 목록에서 영구 누락되고 일정 입력·평가 진입이 막혔다. 배정된 02를 덧붙이고, 관리자 분기와 중복되던 로직을 `planCouncilRows`로 묶었다 | [목록 조회](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilService.java) |
| COUNCIL-035 | ✅ Done | 알림 링크가 협의회ID만 붙여 항상 타당성검토표(Step1)로 향했다. 계획협의회는 그 단계가 없고 Step1 화면에 02 분기가 0건이라 위원 선정·일정 요청·결과서 검토·완료 알림이 빈 화면으로 떨어졌다. 프론트 `council-routing`과 같은 기준으로 진행상태에 맞는 단계 화면을 준다(01~04 검토표, 05~07 개최준비, 그 뒤 결과. 02는 검토표를 건너뛰어 01도 개최준비). 진행상태가 비면 결과 화면으로 보내 알림 발행 자체가 실패하지 않게 했다(COUNCIL-015 후속) | [알림 발행기](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilNotifier.java) |
| COUNCIL-036 | ✅ Done | 계획 상세에서 신청하면 곧바로 개최준비(05)로 올라가, 취소가 열려 있는 작성중·작성완료를 지나가지 않아 잘못 신청해도 되돌릴 경로가 없었다. 계획협의회만 개최준비까지 취소를 허용한다(사용자 결정). 위원 편성·일정 수집 전이라 부작용이 없고 사업 상태 연동도 없어 원장만 지운다. 02는 Step1을 지나지 않아 기존 취소 버튼을 만날 수 없으므로 개최준비 화면에 IT관리자용 버튼과 확인 다이얼로그를 새로 뒀다. 사업 협의회 허용 단계는 그대로 | [취소](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilCancelService.java) · [화면](it_frontend/app/composables/council/usePlanCouncilCancel.ts) |
| COUNCIL-037 | ✅ Done | 결과서 결재 신청서명을 타당성검토표(BPOVWM)에서 찾아 02는 항상 협의회ID로 떨어졌다. 02가 실제로 도달하는 유일한 결재 단계라 결재함에서 바로 보였다. 목록 제목과 같은 `CouncilSubjectName`을 주입해 '정보기술부문계획 수립/조정'이 찍히게 했다 | [결재 서비스](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilApprovalService.java) |
| COUNCIL-038 | ✅ Done | 생략 처리의 사업 상태 갱신에 심의유형 가드가 없었다. 02는 `ABUS_MNG_NO`가 계획번호라 upsert가 계획번호를 키로 한 사업관계 행을 새로 넣을 수 있었다. 결재완료(04) 상태 게이트로 02가 도달하지 못해 실제 피해는 없었으나, 신청·완료·취소와 같은 규칙으로 `markProjectCouncilDone`에 위임해 마지막 무가드 지점을 없앴다 | [생략 처리](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilService.java) |
| COUNCIL-039 | ✅ Done | 화면은 심의유형별로 갈라 보여 주지만 API 직접 호출은 막혀 있지 않았다. ① 계획협의회에 사전·주요 질의응답이 쌓이고 ② 사업 협의회 ID로 계획 전용 조회·저장을 부르면 사업번호를 계획번호로 계획 조회에 넘기고 ③ 계획협의회 위원이 6항목 평가를 직접 호출하면 쓰이지 않을 행이 쌓였다. `CouncilAccessGuard.verifyPlanCouncil`·`verifyNotPlanCouncil`을 두고 네 지점에 걸었다(거부 400) | [접근 가드](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilAccessGuard.java) |
| COUNCIL-040 | ✅ Done | 목록·상세 변환이 계획협의회 원장을 순번 없는 복합키(`BprojmId(계획번호, null)`)로 사업 조회에 넣었다. 늘 빈 결과로 떨어지는지가 구현에 달려 있어 예외가 나면 관리자 목록 전체가 깨진다. 02는 사업 조회를 건너뛰고, 계획번호로 헛돌던 정보보호 소요자원 조회도 함께 건너뛴다. 순번이 없으면 조회하지 않는 `findProjectOf`로 모았다 | [목록·상세 변환](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilService.java) |
| COUNCIL-041 | ✅ Done | 개최준비(05)에서 위원을 저장할 때 새로 평가 의무가 생긴 위원에게 일정입력 요청 알림을 발행한다. 신청 직후 위원이 없어 수신자가 사라지던 02 신규 흐름을 보완하고, 기존 위원에게 같은 알림을 중복 발행하지 않는다 | [위원 저장](it_backend/src/main/java/com/kdb/it/domain/council/service/CommitteeService.java) |
| COUNCIL-042 | ✅ Done | 위원 편성과 일정 확정도 일정 제출과 같은 협의회 원장 비관적 잠금을 사용한다. 위원·일정 집합을 읽기 전에 개최준비 상태를 잠가, 세 작업의 경합이 낡은 자료로 일정확정(06)을 만들지 못하게 했다 | [위원 저장](it_backend/src/main/java/com/kdb/it/domain/council/service/CommitteeService.java) · [일정 확정](it_backend/src/main/java/com/kdb/it/domain/council/service/ScheduleService.java) |
| COUNCIL-043 | ✅ Done | 계획협의회 생성 전에 계획 원장 행을 3초 제한 비관적 잠금으로 잡고 활성 02 협의회를 다시 확인한다. 동시 신청을 계획 단위로 직렬화하고, 없는 계획이나 잠금 시간 초과를 명시적 오류로 처리한다 | [생성 가드](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilPlanCreationGuard.java) · [계획 저장소](it_backend/src/main/java/com/kdb/it/domain/budget/plan/repository/BplanmRepository.java) |
| COUNCIL-044 | ✅ Done | 계획평가 항목에 중첩 Bean Validation을 적용하고 한 요청 안의 사업관리번호 중복을 저장 전에 거부한다. 동일 복합키를 두 번 영속화해 500이 나던 입력을 400 경계에서 차단한다 | [요청 DTO](it_backend/src/main/java/com/kdb/it/domain/council/dto/CouncilDto.java) · [평가 저장](it_backend/src/main/java/com/kdb/it/domain/council/service/PlanEvaluationService.java) |
| COUNCIL-045 | ✅ Done | 계획협의회 개최준비 취소를 활성 위원과 일정이 모두 없는 경우로 제한했다. 서버는 같은 원장 잠금 안에서 하위 행 존재를 확인하고, 화면은 위원 편성이 끝나면 취소 버튼을 숨긴다 | [취소 서비스](it_backend/src/main/java/com/kdb/it/domain/council/service/CouncilCancelService.java) · [화면 상태](it_frontend/app/composables/council/usePlanCouncilCancel.ts) |

코드 변경(COUNCIL-034~040): `it_backend` — `CouncilService`(정보보호관리자 분기 02 추가, `planCouncilRows`·`findProjectOf` 분리, 생략 처리 가드), `CouncilNotifier`(단계별 링크), `CouncilApprovalService`(신청서명), `CouncilAccessGuard`(심의유형 가드 2개), `CouncilCancelService`(02 개최준비 허용), `QnaService`·`MainQnaService`·`PlanEvaluationService`·`EvaluationService`(가드 배선), 테스트 11건 추가와 생성자 인자 보정. `it_frontend` — `usePlanCouncilCancel`(신규), `pages/info/council/prepare/[id].vue`(취소 버튼), `i18n/messages/council.ts`(문구 7개 ko/en). API 응답 계약 변경 없음이라 codegen 불필요. DB 변경 없음.

조사 경위(COUNCIL-034~040): 2026-09-26 사용자 요청으로 심의유형 02를 중심으로 신청·검토표·결재·위원·일정·평가·결과서·Q&A·취소·사업상태연동·목록·알림·권한 13개 흐름을 훑었다. 정상으로 확인한 곳은 신청/재신청, 검토표 제외, 위원 편성(7팀·겸직 04), 서면개최 차단, 평가·완료 판정, 결과서 화면, 목록·라우팅, 알림 제목, 권한 모델이다.

검증 결과(COUNCIL-034~040): 백엔드 `./gradlew test` 5,800건 0 실패 0 오류, `spotlessJavaCheck` 통과. 프론트 `typecheck` 통과(힙 6GB 필요), 아키텍처·i18n·협의회 컴포저블 테스트 137건 통과, `eslint` 통과. 개최준비 페이지는 취소 추가로 800줄을 넘어 컴포저블 분리 후 797줄.

미검증 범위(COUNCIL-034~040): 실제 계정으로 정보보호관리자가 계획협의회 목록·일정·평가에 들어가는 종단 확인, 알림을 눌러 단계 화면으로 이동하는 확인, 개최준비에서 취소 후 계획 상세 재신청, 02 결재함에서 신청서명 표시. 모두 브라우저 확인 대상이다. 039의 거부는 API 직접 호출 경로라 화면에서는 재현되지 않는다.

코드 변경(COUNCIL-041~045): `it_backend` — `CommitteeService`·`ScheduleService`·`CouncilCancelService`·`PlanEvaluationService`·`CouncilDto`·`CouncilAccessGuard`, 계획 생성 잠금을 맡는 `CouncilPlanCreationGuard` 신규, `BplanmRepository` 잠금 조회 추가, 관련 서비스·DTO·저장소 계약 테스트. `it_frontend` — `usePlanCouncilCancel`과 개최준비 페이지의 위원 편성 여부 연결. API 응답·DB 스키마 변경 없음.

검증 결과(COUNCIL-041~045): 백엔드 협의회 도메인·계획 잠금 계약·파일 길이 규칙 544건 통과, `spotlessJavaCheck` 통과. 프론트 취소 노출 회귀 테스트 통과, 6GB 힙 `typecheck`, ESLint(오류 0), 전체 `format:check` 통과. RED 재현 테스트는 별도 커밋으로 보존했다.

미검증 범위(COUNCIL-041~045): 실제 Oracle 두 세션에서 계획협의회 동시 신청 및 위원 편성·일정 제출·확정 잠금 대기, 실제 계정으로 위원 편성 뒤 일정입력 요청 알림 수신, 브라우저에서 위원 편성 뒤 취소 버튼 소멸과 API 취소 거부. 개발계 배포와 메뉴 오픈은 수행하지 않았다.
...
검증 결과(COUNCIL-015 추가분): 백엔드 전체 테스트 5,789건 실행에서 확인된 유일한 실패였던 사업계획 목록 응답 4개 필드(`bizplanExists`·`scheduleCount`·`scheduleStartDate`·`scheduleEndDate`)의 OpenAPI 계약 기준선을 현행화했고 대상 계약 테스트를 통과했다. 최종 결재 콜백이 사업 상태 자동 동기화를 호출하는지도 테스트로 고정했으며 `spotlessJavaCheck`를 통과했다. 프론트는 영향 테스트 131건과 후속 포맷 대상 테스트 15건, ESLint, 전체 `format:check`, 6GB 힙 `typecheck`를 통과했다.
