...
| ID     |     우선순위     | 유형 | 과제                                  | 근거/조건                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------ | :--------------: | ---- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SEC-10 |   🏛️ External    | MFA  | 지정맥 BioAgent 연동 규격 확인        | 내부망 MFA 설계가 참조한 `mfa.md`가 저장소에 없어 BioAgent 요청 페이로드를 확인할 수 없었다. `useMfa.connectAgent()`는 `{command:'identify', trId}` 최소 JSON을 추정으로 보낸다. 서버는 어떤 경우에도 거래 nonce와 `FE00`을 재대조하므로 이 페이로드가 인증 판단 근거는 아니지만, 실제 규격을 받으면 그 페이로드만 교체한다. 규격 입수가 선행 조건이다.                                                                 |
| SEC-11 |    🟡 Medium     | MFA  | 지정맥 결과의 서버 검증 규격 도입     | 제공된 자료에 BioAgent 결과의 서명·기기 증명 규격이 없어 서버는 거래 nonce와 `FE00` 코드만 대조한다. 따라서 지정맥은 FIDO·mOTP보다 위변조 저항성이 낮다(`FingerVeinMfaProvider` JavaDoc). 서버 검증 규격이 제공되면 그 공급자만 교체한다.                                                                                                                                                                            |
