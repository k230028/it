...
| MIG-05 |      🟢 Low       | 이관 | `updateProject`의 `Bitemm` 생성 중복    | `it_backend/src/main/java/com/kdb/it/domain/budget/project/service/ProjectService.java:539`의 신규 품목 생성 분기가 공용 `buildBitemm`(같은 파일 316행, `replaceItemsForMigration`이 사용)을 거치지 않고 `Bitemm.builder()`를 직접 호출해 같은 필드 목록을 손으로 중복한다. 두 경로 중 하나만 고치면 조용히 어긋난다. `updateProject`도 `buildBitemm`을 쓰도록 정리한다.                          |
...
| MIG-07 |     🟠 High       | 테스트 | `test-it` 프로파일의 `MfaProviderRegistry` 미등록으로 통합 테스트 전체 실패 | `MfaConfig.mfaProviderRegistry`가 `@Profile({"local-ext","local-int","dev","prod"})`만 지정하고 `test-it`을 포함하지 않아 `@SpringBootTest` 기반 통합 테스트가 컨텍스트 로딩 단계에서 전부 실패한다(`AuditFailureIsolationIT`로 확인 — `main` 브랜치에서 이 이관 작업 이전부터 실패). 이번 이관 범위와 무관한 기존 결함이며 `MigrationImportIt`은 테스트 전용 폴백 경로를 타서 영향을 받지 않는다. `test-it`을 이 프로파일 목록에 추가할지는 `app.mfa.mock-enabled` 가드와 함께 보안 관련 불변조건을 건드리므로 사용자 확인 후 결정한다. |
| MIG-08 |      🟢 Low       | 문서 | `CostDto` 주석의 `ABUS_TC` 코드값 오기 | `it_backend/src/main/java/com/kdb/it/domain/budget/cost/dto/CostDto.java:558`의 주석은 `abusTc=02(계속)`·`신규(abusTc=01)`이라 적었지만 실제 저장값은 `AdapterSupport.abusTc`·`CostService` 기준 신규=`10`, 계속=`20`이다. 주석만 실제 코드값으로 고친다(비즈니스 로직 변경 없음).                                                                                                    |
| MIG-09 |     🟠 High       | 테스트 | 이관 서비스 클래스의 JaCoCo 커버리지 미달로 `./gradlew check` 적색 | Task 18 Health Stack 검증(`./gradlew clean check --no-daemon`)에서 `jacocoTestCoverageVerification`이 실패했다. `DelegatedBudgetSheetAdapter`(complexity 0.68), `OrgIdentityResolver.Resolution`(branch 0.37/complexity 0.60), `MigrationYearSnapshot`(line 0.22/branch 0.14/complexity 0.25), `MigrationAmounts`(line 0.66), `MigrationIoeCatalogReader`(line 0.05/branch 0.00/complexity 0.14), `MigrationImportService`(line 0.65/branch 0.57/complexity 0.50), `PlanService`(complexity 0.69)가 모두 클래스별 70% 기준에 미달했다 — 대부분 Oracle 의존 분기라 Mockito 단위 테스트만으로는 도달하기 어렵고 실제 커버리지는 `integrationTest`(`MigrationImportIt`)에서 나온다. `./gradlew test`·`spotlessJava`는 통과했으므로 이 게이트만 남았다. 해소하려면 (a) 단위 테스트를 보강하거나 (b) Oracle 통합 테스트로만 검증 가능한 클래스를 다른 Oracle 의존 클래스처럼 `build.gradle`의 `jacocoExcludes`에 추가하는 방안을 검토한다 — 후자는 커버리지 기준 자체를 낮추는 결정이므로 사용자 확인 후 진행한다.                                                                                                    |
