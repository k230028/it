...
| BE-104 | 높음 | ⬜ Open | `main`에 이미 있던 결함이며 이번 동시성 스탬프 브랜치 최종 리뷰에서 드러났다(브랜치가 만든 결함이 아니라, 브랜치가 우연히 가리고 있었을 뿐이다). `it_frontend/app/components/cost/TerminalFormDialog.vue`의 신규 금융정보단말 연결 저장 흐름이 부모 전산업무비 수정 요청에 최소 본문(`tmnYn`·`complete`·`concurrencyStamp`)만 보내는데, `CostService.updateCost`는 요청을 완전 치환으로 처리해 보내지 않은 부모 필드를 null로 지우고(`CostService.java:488`) 그 부모에 속한 기존 금융정보단말 행 전체를 soft delete한다(`CostService.java:567-570`). 병합하면 이 결함이 다시 운영에 노출되어 부모 데이터와 기존 단말 행이 유실된다 | `CostService.updateCost`가 진짜 부분 업데이트가 되도록 바꾸거나, 호출자(`TerminalFormDialog.vue`)가 이미 들고 있는 부모 전산업무비 전체 레코드를 함께 보내도록 고친다 | `it_frontend/app/components/cost/TerminalFormDialog.vue`, `it_backend/src/main/java/com/kdb/it/domain/budget/cost/service/CostService.java:488,567-570` |
| BE-105 | 보통 | ⬜ Open | 같은 신규 금융정보단말 연결 흐름이 `createCost` 호출 뒤 부모 업데이트 호출을 순차로 두 번 보낸다. 두 번째 호출에서 409(동시성 충돌)가 나면 첫 번째 호출(단말 생성)은 이미 반영된 채로 남는데 이 다이얼로그에는 병합 UI가 없어 사용자가 그 상태를 바로잡을 방법이 없다. 두 번의 API 호출로 나눈 설계 자체에 내재한 좁은 half-applied 구간이다 | 두 호출을 하나로 묶거나, 두 번째 호출이 실패했을 때 첫 번째 호출 결과를 사용자에게 안내하고 정리할 방법을 추가한다 | `it_frontend/app/components/cost/TerminalFormDialog.vue` |
