...
| BE-17 | 🟡 Medium | 정책   | 프로젝션 보류 정책 4개 결정 후 재계획                                    | BITEMM GCL 대표행, BBUGTM 대표행, BPROJM 배치 사업명 대표행, BITEMM/BCOSTM 키의 `(sourceNamespace,key)` 분리 정책을 각각 확정해야 한다(BESTTM 물리 PK와 JPA ID 정합화는 `V20260724_001`로 완료 — `TASK_DONE.md` 2026-07-25 절 참조, 단 survey 리포트 §결론에는 미반영 상태). 결정 전에는 기존 엔티티 1조회와 현재 의미를 유지하며, 승인 기준과 차단 경로는 `docs/superpowers/reports/2026-07-be03-projection-survey.md` §결론과 승인 게이트를 따른다. |
