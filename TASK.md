...
| MIG-13 |     🟡 Medium     | 테스트 | 통합 테스트 잔여 실패 8건 (프로젝션 단정·OOM) | MIG-07(`MfaProviderRegistry` 미등록)을 2026-08-16에 해소해 `./gradlew integrationTest` 실패가 48건에서 **8건**으로 줄었다(132건 중 8실패·3스킵). 남은 실패에 `NoSuchBeanDefinitionException`은 없다. 내역은 `*ProjectionIt` 계열의 `AssertionFailedError` 7건(`AdminListProjectionIt`·`LoginHistoryProjectionIt`·`NotificationInboxProjectionIt`·`PlanListProjectionIt`·`ServiceRequestDocVersionProjectionIt`·댓글 트리 조회·가이드 문서 목록 경량 프로젝션)과 Gradle Test Executor의 `OutOfMemoryError` 1건이다. 프로젝션 단정은 로컬 Oracle의 실제 데이터를 읽으므로 **데이터 전제를 고정하지 않은 테스트인지, 실제 계약 위반인지 먼저 가려야 한다** — 전자면 픽스처를 심고, 후자면 쿼리를 고친다. OOM은 `integrationTest` 태스크의 힙·포크 설정으로 분리해 본다. |
