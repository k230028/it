...
| ID    | 우선순위  | 유형   | 과제                                                                     | 근거/조건                                                                                                                                                                                                                                                                                                                                        |
| ----- | :-------: | ------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| BE-03 | 🟡 Medium | 성능   | 경계선 프로젝션 보수적 선적용 후 운영 관측 조정                          | 2026-07-27 방향 확정: 운영 실측을 기다리지 않고 응답 계약을 바꾸지 않는 보수적 프로젝션(응답이 실제 사용하는 컬럼 전체를 담는 view)을 우선 적용하고, 운영 관측(AWR/호출량)으로 컬럼 축소를 후속 조정한다. 1차 대상: Estimate/Application 마스터, 공통코드, 가이드, 게시판 메타·댓글, 조직 전체 목록(6개 묶음). 읽기·쓰기 공유 메서드는 기존 엔티티 메서드를 유지하고 응답 전용 신규 메서드만 추가하며, 신규 조회마다 기존 엔티티 조회와의 결과 동등성 Oracle IT를 선행한다. Project/Cost wide 검색·알림함(3개 묶음)은 응답이 컬럼 전부를 사용해 계약 불변 프로젝션의 이득이 없으므로 응답 계약 분리 결정 후 별도 진행. 근거: `docs/superpowers/reports/2026-07-be03-projection-survey.md` §경계선과 제외 근거. |
