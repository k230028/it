...
| ID    | 우선순위  | 유형   | 과제                                          | 근거/조건                                                                                                                                                                                                                                                                                                                                                       |
| ----- | --------- | ------ | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FE-64 | 🟡 Medium | 검증   | 공통 데이터 이관 API의 OpenAPI codegen 미실행 | 2026-08-29 Task 8(통합 검증) 실행 시점에 로컬 백엔드가 `local-ext`로 기동되지 않았다 — 같은 워킹트리를 공유하는 다른(무관한) 작업의 미커밋 `it_backend/src/main/java/com/kdb/it/domain/migration/terminal/TerminalBulkImportPlanner.java`(untracked, `@Component` 없음)가 `TerminalBulkImportService` 빈 생성에 실패시켜 `ApplicationContext` 로딩이 막혔다(Flyway는 `V20260829_004`까지 정상 적용됨 — 체크섬 문제 아님). 백엔드가 정상 기동 가능해지면 `cd it_frontend; npm run codegen; npm run codegen:check`을 재실행해 `app/types/api.d.ts`에 `/api/admin/migration/common-data*` 경로 3개(export·dry-run·commit)가 반영되고 drift 검사가 통과하는지 확인한다. codegen과 함께, 백엔드가 정상 기동한 같은 환경에서 관리자 로그인 → `/admin/migration/common-data` 진입 → 전체 다운로드 → 받은 파일을 그대로 재업로드 → dry-run 요약이 삭제 0건·추가 0건(전부 갱신)으로 나오는지 확인 → 확정 반영 성공까지 이어지는 동일 서버 왕복 스모크를 1회 수행한다. `npm run codegen`·`npm run codegen:check`와 이 왕복 스모크 모두 첫 운영 사용 전 필수다.                                                                                                                                                                                                                                                                                                                                                       |
