...
| MIG-11 |     🟡 Medium     | 이관 | 편성요청서 반입 후 편성행 생성 절차 문서화 | 부점 편성요청서 반입(`/admin/migration/requests`)은 원장(`BPROJM`·`BITEMM`·`BCOSTM`)과 이관 결재완료 받이까지만 만들고 `BBUGTM`을 만들지 않는다(`RequestFormImportIt.neverCreatesBudgetRows`가 고정). `BudgetRateApplicationService.applyItemRates`가 연도 전량을 논리삭제한 뒤 재삽입하는 구조라 파일마다 부르면 앞서 반입한 편성행이 사라지기 때문이다. 따라서 반입만으로는 예산 편성·현황 화면에 0으로 나온다. 반입을 모두 마친 뒤 예산작업 화면에서 편성률을 한 번에 적용해야 한다는 운영 절차를 사용자 가이드에 명시한다. |
