...
| ID    |   우선순위    | 유형   | 과제                                        | 근거/조건                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----- | :-----------: | ------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FE-37 |  🟡 Medium   | 다국어 | 사이트 전체 고정 문구의 i18n 카탈로그 이관 잔여 | 작업은 `it_frontend`의 `main`에서 이어 간다(옛 작업 브랜치 `codex/sitewide-fixed-copy-i18n`은 main에 이미 병합되어 별도 커밋이 없다). 2026-08-18 기준 `node scripts/check-user-facing-copy.mjs --scope app`이 **209건**을 보고한다(직전 1530건). 새로 완결한 도메인은 협의회(`components/council`·`pages/info/council-request`)·정보기술부문 계획(`pages/info/plan`·`components/plan`·`features/plan`)·관리자(`pages/admin`·`components/admin`·상세 로그)·게시판(`pages/board`·`components/board`)·사업 가이드이며 모두 감사 0건이다. 카탈로그는 `i18n/messages/`에 도메인 파일로 두고 `i18n/locales/{ko,en}.ts`에 등록한다. 화면 단위로 완결해 커밋한다 — 절반만 이관하면 한 화면에 한글·영문이 섞여 이관 전보다 나쁘다. **잔여 209건의 성격**: ① 화면 4개 — `pages/diagnosis`(24)·`pages/audit`(14)·`pages/preparing`(3)·`pages/index`(2), ② composable 9개 — `useDocumentDetailPage`(26)·`tiptap-table/useTableBorders`(25)·`useProjectDetailPage`(19)·`useMigrationPage`(12)·`useApiFetch`(8)·`costListPageHelpers`(8)·`useHwpxExport`(5)·`useCostListPage`(4)·기타 소량, ③ `components/migration` 3개(20), ④ `types/notification`·`types/mfa`(12), ⑤ `utils/projectTimeline`(10)·`utils/common`(2). **선행 판단이 필요한 것은 `pages/diagnosis` 하나다** — 설문 선택지 문자열(`'예'`·`'아니오'`·`'100명 미만'` 등)이 그대로 `calculateResult()`의 분기 값으로 쓰이고 있어, 문구만 옮기면 업무 분기가 깨진다. 코드값과 표시명을 분리하는 리팩터링(선택지에 안정적인 value 부여)이 함께 필요하며 이는 CLAUDE.md §6의 "업무 분기와 저장에는 코드값만" 규칙과 같은 방향이다. 나머지는 기계적 이관이다. |

_2026-07-26 잔여과제 통합 조치의 완료·폐기 이력은 [`TASK_DONE.md`](TASK_DONE.md)를 참조합니다. FE-21과 FE-30①·FE-28②는 같은 문서의 2026-08-06 잔여과제 저비용 배치 1 기록을, FE-36은 2026-08-09 잔여과제 일괄 조치 기록을, FE-38은 2026-08-17 DDL 불필요 잔여과제 배치 기록을, FE-41·FE-42는 같은 날 배치 4 기록을, FE-40은 배치 5 기록을, FE-39·FE-43은 배치 6 기록을 참조합니다._
