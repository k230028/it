...
| ID     | 우선순위  | 유형 | 과제                                                           | 근거/조건                                                                                                                                                                     |
| ------ | :-------: | ---- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SEC-03 | 🟡 Medium | 운영 | 외부 EAI/GWE 발송 채널 운영 코드 확정 후 알림 채널 매핑 재검증 | 현재 GWE `IF_ID` 임시값(`IPPG00000001`)이 dispatcher와 집행 4단계 서비스에 중복되어 있음. 운영팀 확정 후 설정 프로퍼티·공용 상수로 단일화                                     |
| SEC-04 |  🟠 High  | 인증 | Access/Refresh Token 용도 구분과 검증 강제                     | 두 토큰에 용도 claim이 없고 공통 검증이 서명·만료만 확인함. Refresh Token이 Access 경로로 제출되지 않도록 발급·필터·갱신 경로에서 용도를 검증                                 |
| SEC-05 |  🟠 High  | 인가 | 비게시판 업무 파일의 읽기 권한을 부모 자원 기준으로 검증       | 현재 공통게시판 외 파일은 `FileOwnershipChecker.canRead()`가 허용함. 파일 종류별 소유자·부서·부모 업무 권한을 연결하고 목록·메타·다운로드·미리보기에 동일 적용                |
| SEC-06 | 🟡 Medium | 인증 | 로그아웃·Refresh Token 저장·로그 마스킹 정책 보강              | Access Token 만료 후 로그아웃 실패 시 서버 토큰 패밀리가 남고, Refresh Token 원문 저장과 JWT/SSO 토큰 일부 로그가 존재함. 해시 전용 저장·쿠키 기반 폐기·비가역 상관 ID로 전환 |
| SEC-07 | 🟡 Medium | 운영 | 운영 인증 우회 토글과 SSO 세션 고정 방어 검증                  | `allow-bearer-header`, `sso.mock-enabled`를 운영 기동 차단 목록에 포함하고 SSO 성공 경계에서 세션 ID 교체 및 쿠키 정책 검증 필요                                              |
...
| ERR-05 | 🟡 Medium | 신뢰성 | AFTER_COMMIT 알림 실패의 재시도·운영 탐지 경로 추가     | 알림 저장·발송 실패가 원 업무를 롤백하지 않는 정책은 유지하되 outbox/재시도/운영 알람 중 하나로 유실을 탐지·복구해야 함. EAI 실패에도 발송 완료 시각을 기록하는 상태 모델도 함께 분리                                                                                                              |
| ERR-06 |  🟠 High  | 감사   | 감사로그 저장 실패의 지속 탐지 수단 추가                | 원 업무 보호를 위해 `ChangeLogEntityListener`가 감사 실패를 삼키는 정책은 유지하되 메트릭·비재귀 알람으로 감사 추적 유실을 즉시 식별해야 함                                                                                                                                                        |
| ERR-07 | 🟡 Medium | UX     | 정상 빈 상태와 조회·변환 실패를 구분하는 화면 상태 추가 | 장면·변수 카탈로그(`ExcalidrawWrapper`, `TiptapEditor`), 자동완성·통화(`useCostListPage`, `useMentionAutocomplete`, `useProjectCurrencies`), 예산기간·인쇄·사업개요·과거버전(`budget-period`, `plan/[id]`, `bizplan/[abusMngNo]`, `stores/review`)의 TODO 경로에 진단 로그·재시도·인라인 경고 적용 |
