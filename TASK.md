...
| MIG-14 |     🟡 Medium     | 이관 | 편성요청서 1-1 단위 뒤섞임의 과대 기재 미탐지 | 정보화사업 금액 3종 산출(`463565b6`)은 요약표 기재 단위를 1-2 품목 합계와 대사해 역추정한 뒤, `총 사업금액(전체기간)` 칸에 접미사가 없으면 그 배수를 폴백 적용한다(`CapitalProjectFormAdapter.declaredAmounts`). 두 칸의 단위를 뒤섞어 적은 파일에서 **과소 기재는 `DFR < 0` 검산이 잡지만 과대 기재는 잡지 못한다** — 표가 백만 단위, 자유칸이 원 단위면 `TOT_RQM_AMT`가 100만배로 부푼다. `AMOUNT_COLUMN_LIMIT`(10^15, `NUMBER(18,3)` 정수부 15자리) 가드가 컬럼을 넘치는 값만 미적재로 돌려 `ORA-01438`로 파일 전체가 실패하던 경로는 막았으나(2026-08-16 최종 리뷰 I-1), **그 한계 미만의 과대는 탐지 없이 적재된다.** 닫으려면 폴백 경로에 한해 `총액 / ('26년도 합계 + '26년도 이후)` 비율 상한을 두면 되는데, **정상 다년도 사업이 갖는 비율의 상한이 업무 판단**이라 값을 먼저 정해야 한다(단위가 섞이면 5자릿수 이상 벌어지고 정상은 한~두 자릿수로 관측). |
