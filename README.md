...
├── docs/          공통 설계·계획·검증 문서(superpowers·operations·test·db-schema-gap·artifacts)
├── CLAUDE.md      공통 필수 규칙
├── TASK.md        미구현·기술부채
└── TASK_DONE.md   완료·종료 과제 보관
```
...
운영에서는 Micrometer의 `approval.it_budget.preview`·`approval.it_budget.submission.duration` 처리시간과 `approval.it_budget.preview.outcome`·`approval.it_budget.submission` 결과 건수를 확인합니다. 원장 종류별 충돌은 `approval.it_budget.submission.source_changed`, 커밋된 상신의 문서·원장 수는 `approval.it_budget.submission.documents`·`approval.it_budget.submission.sources`, 토큰 서명 실패는 `approval.it_budget.preview.signature_failure`, 과거 v1 조회와 v2 무결성 실패는 `approval.it_budget.snapshot.legacy_read`·`approval.it_budget.snapshot.integrity_failure`로 관측합니다. 결과 태그는 `success`, `invalid`, `expired`, `source_changed`, `stale`, `concurrent_update`, `error`로 제한하며 개인정보·원장 ID를 태그로 사용하지 않습니다. 상신 성공 및 수량은 트랜잭션 커밋 이후에 기록하고 해당 계측 실패가 업무 결과를 바꾸지 않게 합니다. 손상 문서의 메일 요약 생략은 `approval.snapshot.mail.degraded`의 `version` 태그와 경고 로그로 확인합니다.
...
- 국내부점(부점코드가 `9`로 시작하지 않음) 소속 외주직원(행번 `O`/`o`)은 접속 제한 대상으로 수동 로그인·SSO 모두 거부됩니다. 국외점포 소속 외주직원과 일반 행번은 로그인할 수 있습니다. 판정 규칙과 예외 경로는 [백엔드 README](it_backend/README.md#인증과-비동기-부수효과)를 따릅니다.
...
- 2026-09-12: 신청서 PDF 미리보기 서비스워커(`pdf-preview-sw.js`)를 도입 하루 만에 제거하고, 이미 등록된 브라우저의 워커를 해제하는 클라이언트 플러그인 추가
- 2026-09-11: 접속 제한 대상 로그인 차단 — 부서코드가 `9`로 시작하지 않는 외부 행번(`O`/`o`) 사용자는 수동·SSO 로그인을 모두 거부하고 실패 이력에 사유를 남김. 관리자 로그인 이력에 부서명·팀명 표시
- 2026-09-11: 전산업무비 담당자를 작성자 소속 부서 직원으로 제한, 상세 화면 단말기 표 분리, 편성요청서 외화 행의 환율기준일자를 통화 공통코드 코드값상세로 자동 채움, 결재 대시보드 조회 서비스 분리와 bulk 요청 검증, 신청서 PDF 다운로드 파일명 규칙 도입
- 2026-09-10: 화면별 작성 안내 팝업 — 공통 안내 외에 정보화사업·경상사업·전산업무비 작성 화면 진입 시 이어서 표시하는 안내 3종(`common.info`·`common.ordn`·`common.cost`)을 추가하고 문서번호 접두사를 유형별로 분리. [오늘 하루 보지 않기]는 1일 쿠키, [다시 보지 않기]는 `localStorage`
- 2026-09-10: 금융정보단말기 목록 치환 API(`PUT /api/cost/{itMngcNo}/terminals`) — 부모 전산업무비의 업무 필드를 보존한 채 같은 개정본의 단말기만 저장하고, 전년도 불러오기에서 단말기 상세까지 복제. 정보화사업·전산업무비 작성 화면의 자동 임시저장 제거
- 2026-09-09: 반입 원본 파일 다이얼로그 분리와 전산업무비·금융정보단말기 첨부 연동, 리치 HTML 렌더 정화 설정 공통화(`utils/richHtml.ts`)와 본문 내부 링크의 포탈 탭 이동, EAI 전문 시스템환경구분코드(`EAI_SYS_ENV_TC`) 프로파일 분리
- 2026-09-08: 전산업무비 동시성 스탬프·충돌 병합 — 목록·상세·이력 응답에 스탬프를 싣고 낡은 저장은 409로 거부하며, 화면은 3-way 병합 다이얼로그로 해소. 정보화사업(`BPROJM`·`BITEMM`)에도 같은 규약을 적용하고 단말기 연결 전용 엔드포인트 신설
- 2026-09-07: SSO 콜백 무세션화 — 왕복 상태를 `sso-next`/`sso-origin`/`sso-verified` 쿠키로만 운반해 로드밸런서 세션 유지 없이 동작. 지정맥 MFA 거래 상태 관리 개선
- 2026-09-06: 전산예산 신청서 스냅샷 v2 무결성 — 서버 미리보기(HMAC 토큰, 30분)와 원자적 상신, 원장 변경·경합 오류 코드, Micrometer 관측 지표, 결재선 기안자 결재일시 표시. 준정적 캐시 TTL을 1시간에서 60초로 단축하고 알림 중복 발송·재시도 상한 초과를 차단
- 2026-09-04: 전산예산 작성 화면 개선 — 신청서상태 작성완료(`0`) 신설과 수기등록 `9` 이관, 작성 화면 임시저장·저장 버튼(`complete` 플래그)과 작성완료·임시저장 배지, 사업연도 올해·내년 선택(9월 기준 기본값), 소요자원 산정근거 공통코드, 국내점포 결재라인 자동지정 제안 API
- 2026-09-03: 잔여과제 정리 — 특수 문서 관리번호 접두사 분리와 BGDOC 채번 공통화, 예산·QnA 조회 범위 검증, 전산업무비 이력 권한을 서비스 경계로 이동, 단말기 업로드 대상 연도 동적화, 프론트 예산 상세·Tiptap 도구막대 책임 분리
