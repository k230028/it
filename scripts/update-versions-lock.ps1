...
#       C:\it\versions.lock을 통째로 재작성한다(멱등). 릴리스/재현 시 호환 커밋 조합 기록용.
...
function Get-GitValue([string]$repoPath, [string[]]$gitArgs) {
    ...
    if ($LASTEXITCODE -ne 0 -or -not $result) {
        throw "git 명령 실패: 저장소 '$repoPath'에서 'git $($gitArgs -join ' ')' 실행에 실패했습니다. 출력: $result"
    }
...
# versions.lock을 통째로 재작성한다(UTF-8, BOM 없음).
Set-Content -Path $lockPath -Value $lines -Encoding utf8
